...
export function inspectFencedCodeContent(tokens, startIdx, md, options) {
...
export function inspectBlockquoteContent(iterable, startIdx) {
  let content = "";
...
export function renderCalloutPostfix(token, options = {}) {
  const callout = token.attrGet("data-callout");
  const fold = token.attrGet("data-callout-fold");
  if (callout && fold) {
    return "</div></details>";
